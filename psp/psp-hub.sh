...
if [ `whoami` != $USER ]; then
    echo -e "$RED You must run this script with '$USER' user. $END_COLOR"
    exit 1
fi
...
if [ -f $SOURCE_NEW/$YYYYMMDD/$SOURCE_FILE ]; then
    ...
    if [ -d $BACKUP/$YYYYMMDD ]; then
        echo -e "$GREEN Rename the current backup file. $END_COLOR"
    ...
    # deploy
    echo -e "$GREEN Deploy $SERVICE_NAME: Begin. $END_COLOR"

    echo -e "---> Backup source code."
    mv $SOURCE_CURRENT/$SOURCE_FILE $SOURCE_BACKUP/$YYYYMMDD/
    sleep 2

    echo -e "---> Copy new source."
    cp $SOURCE_NEW/$YYYYMMDD/$SOURCE_FILE $SOURCE_CURRENT/$SOURCE_FILE
    sleep 2

    echo -e "---> Build container from image."
    ...
    echo -e "$GREEN Deploy $SERVICE_NAME: Done. $END_COLOR" 
else
    echo -e "$RED New source not found! $END_COLOR"
fi